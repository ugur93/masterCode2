...
class NCNET1_GJOA2(NN_BASE):
    ...
    def update_model(self):
        ...
        self.initialize_model_2()
        weights=old_model.get_weights()
        self.model.set_weights(weights)

    def initialize_model_2(self):
        print('Initializing %s' % (self.model_name))

        for key in self.input_tags:
            n_input = len(self.input_tags[key])
            aux_input, input, merged_out, out = self.generate_input_module_2(n_depth=self.n_depth,
                                                                           n_width=self.n_width,
                                                                           n_input=n_input,
                                                                           n_inception=self.n_inception,
                                                                           l2_weight=self.l2weight, name=key,
                                                                           thresholded_output=self.add_thresholded_output)
            self.aux_inputs.append(aux_input)
            self.inputs.append(input)
            self.merged_outputs.append(merged_out)
            self.outputs.append(out)
    ...
    def generate_input_module_2(self, n_depth, n_width, l2_weight, name, n_input, thresholded_output, n_inception=0):
        ...
        if n_depth == 0:
            ...
            if n_inception > 1:
                # temp_output = add_layers(input_layer, 1, n_width, l2_weight)
                temp_output = generate_inception_module(input_layer, n_inception, self.n_depth_incept,
                                                        self.n_width_incept, l2_weight)
                temp_output = add_layers(temp_output, n_depth, n_width, l2_weight)
                # temp_output = generate_inception_module(temp_output, n_inception, n_depth, n_width, l2_weight)
                # temp_output = add_layers(temp_output, 1, n_width, l2_weight)
            else:
        ...
        if thresholded_output:
            # output_layer = Dense(1, init=INIT, W_regularizer=l2(l2_weight), b_regularizer=l2(l2_weight), bias=True)(
            #    temp_output)
            output_layer = Dense(1, init=INIT, activation='relu', W_regularizer=l2(l2_weight), bias=True)(
                temp_output)
            #
            aux_input, merged_output = add_thresholded_output(output_layer, n_input, name)
        else:
