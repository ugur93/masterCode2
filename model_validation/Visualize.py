    ...
    for output_tag in output_cols:
        ...
        for input_tag in input_cols:
            if input_tag.split('_')[0] == output_tag.split('_')[
                ...
                i += 1
                plt.scatter(self.X_SCALE * X_train[input_tag], self.SCALE * Y_train[output_tag], color='blue',
                            label=output_tag + '_true - train')
                plt.scatter(self.X_SCALE * X_train[input_tag], self.SCALE * self.predict(X_train, output_tag),
                            color='black', label=output_tag + '_pred - train')

                plt.scatter(self.X_SCALE * X_test[input_tag], self.SCALE * Y_test[output_tag], color='red',
                            label=output_tag + '_true - test')
                plt.scatter(self.X_SCALE * X_test[input_tag], self.SCALE * self.predict(X_test, output_tag),
                            color='green', label=output_tag + '_pred - test')
...
def plot_true_and_predicted_zeros(self, X_train, X_test, Y_train, Y_test, output_cols=[]):
    ...
    for output_tag in output_cols:
        ...
        plt.scatter(Y_train.index[ind_train], self.SCALE * Y_train[output_tag][ind_train], color='blue',
                    label='true - train')
        plt.scatter(Y_train.index[ind_train], self.SCALE * self.predict(X_train[ind_train], output_tag), color='black',
                    label='pred - train')
