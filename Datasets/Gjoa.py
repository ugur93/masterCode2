...
def fetch_gjoa_data():
    ...
    if False:
        ...
        cols=['F1_CHK']#,'D3_CHK','B2_CHK']
        MAP_cols={'F1_CHK':'G3 choke opening','E1_QGAS':'G3 gas flow rate'}
        COLOR_MAP={'E1_CHK':'red','F1_CHK':'black'}
        ...
        for i, key in zip(range(0, len(cols)), cols):
            i=0
            try:
                axes[i].scatter(X['time'], GjoaData.X[key],label=MAP_cols[key],color=COLOR_MAP[key])
            except(KeyError):
                axes[i].scatter(X['time'], GjoaData.Y_transformed[key],label=MAP_cols[key],color=COLOR_MAP[key])
